...
const Playlist: React.FC = () => {
  ...
  const handleDeleteTrack = (trackId: string) => {
    if (playlistContext.currentPlaylist) {
      playlistContext.removeTrackFromPlaylist(
        playlistContext.currentPlaylist.id,
        trackId,
      );
    }
  };
  ...
  const handlePlayPlaylist = async (playlist: any) => {
    ...
    if (
      ...
      const updatedPlaylist = playlistContext.playlists.find(
        (p) => p.id === playlist.id,
      );
  ...
  const handlePlayTrack = async (trackUri: string) => {
    try {
      ...
      if (trackUris.length > 0) {
        ...
        const clickedTrackUriIndex = trackUris.findIndex(
          (uri) => uri === trackUri,
        );
      ...
      <div style={{ overflowY: "scroll", marginRight: "20px", flex: 1 }}>
        ...
        {playlistContext.playlists.length > 0 ? (
          ...
          >
            ...
            >
              {playlistContext.playlists.length} Playlist Found
              {playlistContext.playlists.length !== 1 ? "s" : ""}
            </div>
                ...
                style={{
                  ...
                  borderBottom:
                    ...
                    playlistContext.playlists[
                      playlistContext.playlists.length - 1
                    ].id
                      ? "none"
                      : "1px solid #f0f0f0",
                  cursor: "pointer",
                  ...
                  color: "white",
                  backgroundColor:
                    playlistContext.currentPlaylist?.id === playlist.id
                      ? colorScheme.gradientColorA
                      : "transparent",
                  borderLeft:
                    playlistContext.currentPlaylist?.id === playlist.id
                      ? "4px solid " + colorScheme.hoverColor
              ...
              >
                ...
                >
                  ...
                  <button
                    ...
                    style={{
                      padding: "6px 10px",
                      backgroundColor: playlist.isSpotifyPlaylist
                        ? "#6c757d"
                        : "#dc3545",
                      color: "white",
                      border: "none",
                      borderRadius: "4px",
                      cursor: playlist.isSpotifyPlaylist
                        ? "not-allowed"
                        : "pointer",
                      fontSize: "12px",
                      fontWeight: "bold",
                    ...
                    title={
                      playlist.isSpotifyPlaylist
                        ? "Cannot delete Spotify playlists"
                        : "Delete playlist"
                    }
                    onMouseEnter={(e) => {
                      if (!playlist.isSpotifyPlaylist) {
        ...
        <div style={{ flex: 1, display: "flex", flexDirection: "column" }}>
          ...
          {playlistContext.currentPlaylist.tracks.length === 0 ? (
            ...
            >
              ...
              <div style={{ fontSize: "14px" }}>
                Search for songs above to add them to this playlist!
              </div>
            </div>
          ) : (
            <div
            ...
            >
              ...
              >
                <span>
                  ...
                  {playlistContext.currentPlaylist.tracks.length !== 1
                    ? "s"
                    : ""}
                </span>
